...
build-launcher-arm64:generate
	@echo "Building picoclaw-launcher-arm64 for $(PLATFORM)/$(ARCH)..."
	@mkdir -p $(BUILD_DIR)
	GOOS=linux GOARCH=arm64 PLATFORM=linux $(MAKE) -C web build \
                OUTPUT="$(CURDIR)/$(BUILD_DIR)/picoclaw-launcher-$(PLATFORM)-$(ARCH)$(EXT)" \
                WEB_GO='$(WEB_GO)' \
                GO_BUILD_TAGS='$(GO_BUILD_TAGS)' \
                LDFLAGS='$(LDFLAGS)'
	@$(LNCMD) picoclaw-launcher-$(PLATFORM)-$(ARCH)$(EXT) $(BUILD_DIR)/picoclaw-launcher$(EXT)
	@echo "Build complete: $(BUILD_DIR)/picoclaw-launcher-arm64$(EXT)"
